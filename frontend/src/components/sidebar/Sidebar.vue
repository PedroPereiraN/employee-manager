...
<template>
  ...
  >
    <!-- Header -->
    <div
      class="flex items-center gap-3 px-4 py-5 border-b border-gray-700/60"
    ...
    >
      ...
      <button
        ...
        class="p-1.5 rounded-md text-gray-400 hover:text-gray-100 hover:bg-gray-700/60 transition-colors cursor-pointer"
    ...
    <nav class="flex-1 px-2 py-4 space-y-1 overflow-y-auto overflow-x-hidden">
      ...
      >
        <Icon :icon="path.icon" class="shrink-0 transition-colors" width="18" height="18" />
    ...
    <div class="px-2 py-3 border-t border-gray-700/60">
      <button
        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer"
        :class="{ 'justify-center': collapsed }"
        type="button"
        @click="logout()"
      >
